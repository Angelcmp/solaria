...
- **Fase A — aarch64 publicado (v0.9.2, esta sesión)**: `release.yml` con matrix x86_64 + aarch64 (`ubuntu-24.04-arm`) y job `publish` único (assets planos + `SHA256SUMS.txt` conjunto); `install.sh` resuelve assets por `uname -m` (falló el primer intento: download preservaba rutas → sums vacío; corregido aplanando + release v0.9.2 recreado). Validado: binario ELF aarch64 real, `.deb` arm64, checksums OK, one-liner en Ubuntu ARM emulado (rc=0, 582s bajo qemu, `solaria 0.9.2` responde) y regresión x86_64 verde.
- **Fase B — macOS Apple Silicon publicado (v0.9.3, esta sesión)**: `install-macos.sh` nuevo (tarball `darwin-aarch64` verificado con `shasum`, user-local sin sudo ni deps, PATH en `.zshrc`, skip-if-current, uninstall total); CLI portable (`read_daemon_pid` vía `sysinfo`, `home_dir` con fallback, URL de instalador por OS con override); `release.yml` job `macos-14` con smoke CLI + `.dmg`. Incidente: primer build mac falló por `dead_code` (`DEFAULT_INSTALL_SH_URL` sin uso en mac + `-D warnings`); corregido con `cfg`, tag recreado. Validado: Mach-O arm64 real (19 MB), checksums OK, smoke CI (`solaria 0.9.3` en runner), wrapper bash3-compatible. Pendiente: prueba real en un Mac (Gatekeeper: clic derecho → Abrir) + firma Apple a futuro.
- **Docs web**: `docs/INSTALACION.md` nuevo (espejo de instalación para solaria.im: requisitos, one-liners, tiempos reales, update/uninstall, troubleshooting); `README.md`: ejemplo `SOLARIA_VERSION` a v0.9.3 + nota PATH/`command not found`. Providers y versión cruzados con código.
- **Fase C — Windows x64 publicado (v0.9.4, esta sesión)**: `install.ps1` (`irm|iex`, tarball `win-x86_64` con `Get-FileHash`, user-local sin admin, skip/force/uninstall total) + wrapper `scripts/solaria.ps1`; CLI portable (`taskkill`, detached serve, `exec` a `install.ps1`, URL por OS); `tools` con `run_sh()` + pista Git-for-Windows; CI `windows-latest` (NSIS + `.msi` + tarball) con smoke. Incidentes: `DEFAULT_INSTALL_SH_URL` huérfana en mac (inline), smoke por `$LASTEXITCODE` no fiable (a salida), sums `.msi` con espacio (republicado). Validado: `cargo test` 40/40, sintaxis PS vía parser, checksum válido/mismatch, uninstall, resolución real de assets v0.9.4, release verde con las 4 plataformas. Pendiente: prueba en Windows real.
- **Fase G — GUI-only (v0.9.5, esta sesión)**: `cli.rs` eliminado (terminal solo lanza: `solaria` a pelo abre la ventana); `main.rs` simplificado + plugin `single-instance` (enfoca ventana existente); botones en Configuración → Aplicación (`app_version`, `check/install_app_update`, `restart_app`, `uninstall_app` por OS con `winreg` en Windows); updater Tauri firmado (claves generadas, privada en secret, `latest.json` estático en Releases, `createUpdaterArtifacts`); instaladores sin CLI (fichero `VERSION` en tarballs, stop por pkill/taskkill, verify por ficheros; wrappers a lanzadores); smokes CI por artefacto. Incidentes: `get_webview_window` sin `Manager` en scope, `app` sin uso en `uninstall_app` (`-D warnings`), smoke mac (`Mach-O 64-bit executable arm64`), `.sig` no recogidos en linux/win (rebuild con tag recreado), mapper `latest.json` (darwin-x86_64 erróneo → 4 plataformas OK). Validado: `cargo test` 38/38 sin warnings, `tsc` + `npm test` OK, e2e mock + one-liner real v0.9.5 en Ubuntu (rc=0).
