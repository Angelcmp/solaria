---
name: deep-research
description: Pipeline for investigating topics systematically. Use when user asks to "investigate", "research", "analyze", "write a report", "investigar", "analizar", "buscar información", or "hacer un reporte".
---

## Deep Research Pipeline

When the user requests research on a topic, follow this pipeline **without asking unnecessary questions**. Respond in the same language the user is using. Assume the user's request is clear enough and proceed.
...
4. **If `fetch_url` fails** (error, 404, PDF, timeout):
   - Note briefly in the user's language: "Could not access [source]" / "No se pudo acceder a [fuente]"
...
3. **If a source was inaccessible**, cite via snippet: `"According to [source]: [snippet]"` / `"Según [fuente]: [snippet]"`. Do NOT fabricate data.
4. **If search returns no useful data**, state in the user's language: "No specific information found about [topic]" / "No se encontró información específica sobre [tema]".

### Phase 4 — Deliver
1. Present findings directly in the chat with clean markdown formatting in the user's language.
