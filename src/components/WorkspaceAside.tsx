...
function groupConversations(convs: Conversation[]) {
  ...
  if (pinned.length > 0) {
    groups.push({ label: 'Anclados', convs: pinned })
  }
    ...
    <>
      ...
      {!isCollapsed && (
        <div className="flex flex-col bg-[#1C1B1B] border-r border-[rgba(255,255,255,0.04)] overflow-hidden transition-all duration-250 w-[260px] shrink-0">
          ...
          <div className="flex-1 overflow-y-auto py-1" style={{ scrollbarWidth: 'none' }}>
            {conversations.length === 0 ? (
              <div className="text-center py-8 px-4">
                <p className="text-[0.75rem] text-[#666666]">Sin conversaciones</p>
                <p className="text-[0.625rem] text-[#666666] opacity-70 mt-1">Crea una nueva para empezar</p>
              </div>
            ) : (
              groups.map(group => (
                    ...
                    >
                          ...
                          <div className="flex items-center gap-1.5">
                            {conv.type === 'agent' ? (
                              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#00E5C9" strokeWidth="2" className="shrink-0" style={{ filter: 'drop-shadow(0 0 4px rgba(0,229,201,0.3))' }}>
                                ...
                                <circle cx="8" cy="13" r="1.5" fill="#00E5C9"/>
                                <circle cx="16" cy="13" r="1.5" fill="#00E5C9"/>
                                <path d="M12 3v3M12 16v3"/>
                              </svg>
                            ) : (
                              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#DCB263" strokeWidth="2" className="shrink-0">
                        ...
                        {conv.toolSummary && Object.keys(conv.toolSummary).length > 0 && (
                          <div className="flex items-center gap-1 mt-0.5 pl-[19px]">
                            {Object.entries(conv.toolSummary).map(([tool, count]) => (
                              <span key={tool} className="text-[0.5rem] text-[#4a4a4a] font-mono">
                                {tool}×{count}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className={`flex items-center justify-center w-5 h-5 rounded hover:bg-[rgba(220,178,99,0.15)] hover:text-[#DCB263] transition-colors ${
                            conv.pinned ? 'text-[#DCB263]' : 'text-[#666666]'
                          }`}
          ...
          <div className="flex items-center gap-2 px-3 py-2 border-t border-[rgba(255,255,255,0.06)]">
            <button
              onClick={onToggle}
              className="flex items-center justify-center w-6 h-6 rounded hover:bg-[rgba(255,255,255,0.06)] text-[#666666] hover:text-[#E5E5E5] transition-colors"
              title="Colapsar"
            >
      ...
      {isCollapsed && (
        <div className="flex flex-col items-center gap-3 py-3 px-1.5 bg-[#1C1B1B] border-r border-[rgba(255,255,255,0.04)] w-[48px] shrink-0 transition-all duration-250">
          ...
          <button
            onClick={onNew}
            className="flex items-center justify-center w-8 h-8 rounded-md bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] text-[#E5E5E5] hover:bg-[rgba(255,255,255,0.08)] hover:text-white transition-all"
            title="Nueva conversación"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          </button>
          <button
            onClick={onToggle}
            className="mt-auto flex items-center justify-center w-6 h-6 rounded hover:bg-[rgba(255,255,255,0.06)] text-[#666666] hover:text-[#E5E5E5] transition-colors"
            title="Expandir"
          >
